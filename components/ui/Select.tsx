...
>(({ className, children, ...props }, ref) => {
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex w-full items-center justify-between h-10 px-4 py-2",
...
>(({ className, children, position = "popper", ...props }, ref) => {
  const contentBase =
    "relative z-50 min-w-[8rem] overflow-hidden bg-white border border-gray-200 rounded-md shadow-lg";
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ref={ref}
        className={cn(contentBase, positionClasses, className)}
        position={position}
        {...props}
      >
